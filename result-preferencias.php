<?php
	...
	$universitarios = $_POST['universitario'];
	$tematicos = $_POST['tematico'];
?>
    ...
    <body>
		...
		<div class="form-group col-xs-10">
	        <div class="feed">
	          	<div class="container">
					<?php
						$result = array_merge($musicas, $comes, $ambientes, $universitarios, $tematicos);
						for ($i=0;$i<count($result);$i++){ 
						   $sql_cat = "SELECT * 
						               FROM eventoCategoria
						               WHERE cod_item_fk = '$result[$i]'";

						  $resultado_cat = mysqli_query($con, $sql_cat);

						  $row = mysqli_num_rows($resultado_cat);

						  if ($row > 0) {

						    while($lista_cat=mysqli_fetch_array($resultado_cat)){

						      $eventoCat = $lista_cat['cod_evento_fk'];

							   $sql = "SELECT *
							            FROM evento
							            WHERE cod_evento = '$eventoCat'
